...
match *match_descriptors(descriptor *a, int an, descriptor *b, int bn, int *mn)
{
    ...
    *mn = an;
    ...
    for(j = 0; j < an; ++j){
        ...
        m[j].ai = j;
        m[j].bi = best_index; // <- should be index in b.
        m[j].p = a[j].p;
...
point project_point(matrix H, point p)
{
    ...
    float x_proj = result.data[0][0] / result.data[2][0];
    float y_proj = result.data[1][0] / result.data[2][0];

    free_matrix(c);
    free_matrix(result);

    return make_point(x_proj, y_proj);
}
...
image cylindrical_project(image im, float f)
{
    ...
    float xc = original_width / 2;
    float yc = h / 2;
    float new_xc = new_width / 2;
        ...
        for (int y = 0; y < h; ++y) {
            for (int ch = 0; ch < c; ++ch) {
                ...
                if (xp >= 0 && xp < original_width && yp >= 0 && yp < h) {
                    float val = bilinear_interpolate(im, xp, yp, ch);
                    set_pixel(cproj, x, y, ch, val);
                }
            }
        }
    }
